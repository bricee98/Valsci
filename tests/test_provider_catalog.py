...
def test_seed_catalog_uses_configured_model_for_local_provider(monkeypatch, tmp_path: Path):
    monkeypatch.setattr(Config, "LLM_PROVIDER", "local", raising=False)
    monkeypatch.setattr(Config, "LOCAL_BACKEND", "ollama", raising=False)
...
def test_local_default_provider_repairs_openai_seeded_model_list(monkeypatch, tmp_path: Path):
    monkeypatch.setattr(Config, "LLM_PROVIDER", "local", raising=False)
    monkeypatch.setattr(Config, "LOCAL_BACKEND", "ollama", raising=False)
    monkeypatch.setattr(Config, "LLM_EVALUATION_MODEL", "llama3.1:8b", raising=False)
    monkeypatch.setattr(Config, "MODEL_REGISTRY_OVERRIDES", {}, raising=False)
